...
if __name__ == "__main__":
    ...
    print(f"Weight stats - Mean: {regular_conv.weight.mean():.4f}, Std: {regular_conv.weight.std():.4f}")
    ...
    print(f"TConv2d output stats - Mean: {tconv_output.mean():.4f}, Std: {tconv_output.std():.4f}")
    ...
    with torch.no_grad():
        q_nograd, alpha, mask, delta = ternary_quantize_conv(tconv.weight, tconv.thresh_ratio)
    ...
    for thresh in [0.5, 0.75, 0.9]:
        tconv_test = TConv2d(3, 16, kernel_size=3, stride=1, padding=1, bias=True, thresh_ratio=thresh)
