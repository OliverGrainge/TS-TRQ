...
def ternary_quantize_conv(w: torch.Tensor, thresh_ratio: float = 0.75):
    ...
    delta = thresh_ratio * abs_w.mean(dim=(1, 2, 3), keepdim=True)  # Shape: (out_channels, 1, 1, 1)
...
class TSVDConv2d(nn.Conv2d):
    ...
    def _init_weights(self, rank: int = None):
        ...
        if not torch.isfinite(E_flat).all():
            ...
            E_flat = torch.where(torch.isfinite(E_flat), E_flat, torch.zeros_like(E_flat))
    ...
    def forward(self, x):
        ...
        if self.rank > 0 and L.numel() != 0 and R.numel() != 0:
            ...
            E_lr_flat = (self.lr_scalars.view(-1, 1).to(x.dtype) * L) @ R  # (out_channels, flattened_features)
...
# ---------- UTILITY FUNCTIONS ----------
def replace_conv2d_with_ternary_svd(module, rank=8, thresh_ratio=0.75, reg_scale=0.5, reg_type="l1", skip_names=None):
    """
...
if __name__ == "__main__":
    ...
    print(f"TSVDConv2d output stats - Mean: {y_tsvd.mean():.4f}, Std: {y_tsvd.std():.4f}")
