...
if __name__ == "__main__":
    def replace_linear_with_ternary_svd(module, rank=8, thresh_ratio=0.75, reg_scale=0.5, reg_type="l1", skip_names=None):
        """
        ...
        for name, child in module.named_children():
            ...
            if isinstance(child, nn.Linear) and not isinstance(child, TSVDLinear):
                ...
                tlinear = TSVDLinear.from_linear(
                    child, rank=rank, thresh_ratio=thresh_ratio, 
...
if __name__ == "__main__":
    ...
    print(f"TSVDLinear output stats - Mean: {y_tsvd.mean():.4f}, Std: {y_tsvd.std():.4f}")
    ...
    for scale in [0.1, 0.5, 1.0, 2.0]:
        tsvd_test = TSVDLinear.from_linear(linear, rank=8, reg_scale=scale, reg_type="l1")
    ...
    for thresh in [0.5, 0.75, 0.9]:
        ...
        print(f"  Threshold {thresh}: Sparsity {sparsity:.2%}, Error {error:.6f}, Output std {output.std():.4f}")
    ...
    orig_params = linear.weight.numel() + (linear.bias.numel() if linear.bias is not None else 0)
    
    # TSVDLinear parameters
    tsvd_params = (tsvd_linear.weight.numel() + 
    ...
    for batch_size in [1, 4, 16, 64]:
        ...
        print(f"  Batch size {batch_size}: Output shape {y_test.shape}, Mean {y_test.mean():.4f}")
